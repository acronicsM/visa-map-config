...
- [x] Блок под картой «Направления по фильтрам» (`TravelCollections`): список
...
- Фильтры «тип отпуска» и «город вылета» в сайдбаре пока не связаны с картой и
  с лентой направлений (не участвуют в `passesComposite` в `VisaMap`)
...
- Ключ MapTiler задаётся в `.env.local` (`NEXT_PUBLIC_MAPTILER_KEY`); перед продом — ротация и ограничения по referrer
